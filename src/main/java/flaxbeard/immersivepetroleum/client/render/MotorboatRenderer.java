...
public class MotorboatRenderer extends EntityRenderer<MotorboatEntity>{
	...
	public void render(MotorboatEntity entity, float entityYaw, float partialTicks, PoseStack matrix, MultiBufferSource bufferIn, int packedLight){
		matrix.pushPose();
		{
			...
			this.motorBoatResources.setRotationAngles(entity, partialTicks, 0.0F, -0.1F, 0.0F, 0.0F);
			...
			if(entity.hasPaddles){
				VertexConsumer vbuilder_normal = bufferIn.getBuffer(this.modelBoat.getRenderType(texture));
			...
			VertexConsumer vbuilder_armored = bufferIn.getBuffer(this.modelBoat.getRenderType(textureArmor));
	...
	public void setupRotation(MotorboatEntity boat, float entityYaw, float partialTicks, PoseStack matrix){
		...
		if(f > 0.0F){
			matrix.mulPose(new Quaternion(Mth.sin(f) * f * f1 / 10.0F * (float) boat.getForwardDirection(), 0.0F, 0.0F, true));
		}
